...
def find_replacement_for_person(person_id: int) -> dict | None:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            '''
            SELECT p.id, p.full_name, p.role, p.home_base_id,
                   b.priority AS home_priority,
                   b.name AS base_name
            FROM personnel p
...
def record_absence(full_name: str, reason: str, reported_by: str) -> dict:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            'SELECT id, full_name, role FROM personnel WHERE lower(full_name) = lower(%s)',
            (full_name.strip(),),
        )

        person = conn.fetchone()

        if not person:
            return {
                'ok': False,
                'message': f'No encontré a {full_name} en la dotación.'
            }

        conn.execute(
            "UPDATE personnel SET status = 'absent' WHERE id = %s",
            (person['id'],),
        )
...
def register_alert(description: str, location: str, source: str = 'telegram') -> str:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            '''
            INSERT INTO alerts(source, description, location, created_at)
            VALUES (%s, %s, %s, %s)
            ''',
...
def rebalance_staff() -> RebalanceResult:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            'UPDATE assignments SET active = 0 WHERE active = 1'
        )
        ...
        available_people = conn.fetchall()
        ...
        conn.execute(
            '''
            SELECT id, code, vehicle_type, current_base_id
            FROM vehicles
            WHERE status = 'available'
            '''
        )

        vehicles = [dict(v) for v in conn.fetchall()]
        ...
        for base in bases:
            ...
            expected_vehicle = _choose_vehicle(
            ...
            vehicle = next(
                (
                    ...
                    and v['vehicle_type'] == expected_vehicle
                ),
                None,
            )
        ...
        for base in bases:
            ...
            assigned_count = conn.fetchone()['total']

            if assigned_count < base['min_staff']:
                uncovered.append(
                    f"{base['name']} ({assigned_count}/{base['min_staff']})"
                )
...
def get_operational_status() -> str:
    ...
    with get_connection() as conn:
        ...
        for base in bases:
            ...
            absent = conn.fetchone()['total']

            vehicle_text = 'Sin vehículo asignado'

            if staff and staff[0]['vehicle_code']:
                vehicle_text = (
                    f"{staff[0]['vehicle_code']} "
                    f"({staff[0]['vehicle_type']})"
                )

            lines.append(
                f"\n• {base['name']} | prioridad {base['priority']} "
                f"| mínimo {base['min_staff']} "
                f"| vehículo {vehicle_text}"
            )
...
def reset_demo_data() -> str:
    ...
    with get_connection() as conn:
        ...
        conn.execute(
            "UPDATE personnel SET status = 'available'"
        )
